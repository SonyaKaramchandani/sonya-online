// source: https://gistcdn.githack.com/martinlaxenaire/549b3b01ff4bd9d29ce957edd8b56f16/raw/2f111abf99c8dc63499e894af080c198755d1b7a/TextTexture.js
...
export class TextTexture {
	...
	writeTexture(measureText = true) {
		...
		const totalHeight =
			this.lines[0][0].position.y + this.lines[this.lines.length - 1][0].position.y + fontSize;
		...
		this.lines.forEach((line, index) => {
			line.forEach((word) => {
				...
				this.onAfterWordWriting &&
					this.onAfterWordWriting(this.context, word, offset, this.content.style, index);
			});
		});
